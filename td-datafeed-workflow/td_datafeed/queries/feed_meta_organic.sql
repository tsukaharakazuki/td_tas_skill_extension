SELECT
    ...
    END AS description,
    image_link,
    CONCAT(link, '?utm_source=', '${channels.meta_organic.utm_source}', '&utm_medium=', '${channels.meta_organic.utm_medium}', '&utm_campaign=', brand_abb, '_', '${channels.meta_organic.utm_campaign_suffix}') AS link,
    title,
...
WHERE
    CAST(year AS INTEGER) >= CAST('${rules.organic_min_year}' AS INTEGER)
    AND CAST(stock_quantity AS BIGINT) > 0
