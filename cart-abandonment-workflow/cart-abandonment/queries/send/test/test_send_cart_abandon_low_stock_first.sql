-- ============================================================
-- test_send_cart_abandon_low_stock_first.sql
-- 初回テスト配信用SQL（カート放棄 在庫僅少版: 2通のみ）
--
-- ⚠️ テスト専用: 実顧客には一切配信しない
--    email を test-recipient+<alias>@example.com に完全置換
--
-- 初回テスト: 最も差が出る2パターンで表示確認
--   1. f2plus_all4_ps_all4_pts : 全スロット埋まり＋ポイントあり（最大表示）
--   2. f2plus_none_ps_none_nopts : 全スロット空（最小表示）
-- ============================================================
...
sampled AS (
  ...
  WHERE rn = 1
    AND (
      (reco_pattern = 'all4' AND ps_pattern = 'ps_all4' AND pts_pattern = 'pts')
      OR
      (reco_pattern = 'none' AND ps_pattern = 'ps_none' AND pts_pattern = 'nopts')
    )
)
