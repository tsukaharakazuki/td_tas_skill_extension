...
```bash
# API経由で実際のアクティベーション一覧と各接続先・IDを抜き出して確認するスクリプト例
...
payload = {
    ...
    "attributes": {
        "activationParams": {
            "name": "[アクティベーション名]",
            "connectionId": "[CONNECTION_ID]",     # 例: 381856 (Engage Studio V1)
            ...
            "timezone": "Asia/Tokyo",
            "notifyOn": ["onFailure"],
            "emailRecipients": [[NOTIFY_USER_ID]],  # 例: 187984 (通知先ユーザーID)
            "connectorConfig": {
                "campaignType": "email",
...
rfm_condition = {
    ...
    "leftValue": {
        ...
        "source": {"name": "[BEHAVIOR_TABLE_NAME]"} # 例: behavior_ms_behavior_rfm_mix
    },
...
parent_segment: "[親セグメント名] (例: MIXオーディエンス)"
audience_id: [AUDIENCE_ID] (例: 983083)

# 2. Engage ワークスペース情報
workspace: "[Engageワークスペース名] (例: TSI)"
workspace_id: "[EngageワークスペースID] (例: 01997f0c-4970-7fe3-a7dc-c99c6e9d85eb)"

# 3. コネクション情報 (Engage V1 Connection)
connection_id: "[CONNECTION_ID] (例: 381856)"

# 4. 常時稼働キャンペーン (AOC) ID一覧
# (例: 休眠復帰・経過日数別の配信など、複数AOCがある場合にメモ)
aoc_ids:
  stage_1: "[AOC_ID_1] (例: 019f5fbc-a3f4-70cf-...)"
  stage_2: "[AOC_ID_2]"
  stage_3: "[AOC_ID_3]"

# 5. ジャーニー ID一覧
journey_ids:
  reengagement: [JOURNEY_ID_1] (例: 328283)
  onboarding: [JOURNEY_ID_2] (例: 334131)

# 6. エラー・失敗時の通知先メールアドレス
email_recipients: [[RECIPIENT_USER_ID_1], [RECIPIENT_USER_ID_2]] # 例: [187984]
```
